(()=>{
  ...
  const isTracing=/tracing\.html(?:$|[?#])/.test(location.href);
  const isStudioPage=isWordSearch||isColoring||isTracing;
  ...
  if(!isStudioPage&&window.FenixCore){
    ...
    window.openFpmPage=id=>{
      ...
      if(mod==='word-search-studio'){location.href='word-search.html?id='+encodeURIComponent(id);return}
      if(mod==='coloring-studio'){location.href='coloring.html?id='+encodeURIComponent(id);return}
      if(mod==='complete-picture'){location.href='complete-picture.html?id='+encodeURIComponent(id);return}
      if(mod==='tracing-studio'){location.href='tracing.html?id='+encodeURIComponent(id);return}
    ...
    const enhanceProjectCards=()=>{
      ...
      [...list.querySelectorAll('.page')].forEach((card,index)=>{
        ...
        const mod=moduleOf(page);
        if(mod==='tracing-studio'){
          const title=card.querySelector('b');if(title)title.textContent=title.textContent.replace('tracing-studio','TR');
          const smalls=card.querySelectorAll('small');if(smalls.length)smalls[smalls.length-1].textContent='Dotknij, aby edytować';
          card.setAttribute('role','button');card.setAttribute('tabindex','0');card.style.cursor='pointer';card.onclick=e=>{if(e.target.closest('button'))return;location.href='tracing.html?id='+encodeURIComponent(page.id)};
        }
        if(!card.querySelector('.fpm-page-edit')&&['maze-studio','word-search-studio','coloring-studio','complete-picture','tracing-studio'].includes(mod)){
          const hint=document.createElement('span');hint.className='fpm-page-edit';hint.textContent='Edytuj →';hint.style.cssText='display:block;margin-top:8px;font-weight:900;color:#e45520';card.appendChild(hint);
          card.addEventListener('keydown',event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();card.click()}});
        }
      });
    };
    enhanceProjectCards();
    const list=document.getElementById('pageList');if(list)new MutationObserver(enhanceProjectCards).observe(list,{childList:true});
    window.addEventListener('fenix-state-change',()=>setTimeout(enhanceProjectCards,0));
  }

  if(!isStudioPage){
    const tiles=document.querySelector('#home .tiles');
    if(tiles&&!tiles.querySelector('[data-fpm-tracing-tile]')){
      const btn=document.createElement('button');btn.dataset.fpmTracingTile='1';btn.innerHTML='<b>TR · Tracing Studio</b><small>Pełna logika FP + edycja projektu</small>';btn.onclick=()=>location.href='tracing.html';tiles.appendChild(btn);
    }
  }
  ...
  bar.innerHTML=`
    ...
    <div class="fpm-bar-content" aria-hidden="true">
      ...
      <div class="fpm-module-strip" role="list">
        ...
        <button data-shell-target="tracing" role="listitem"><b>TR</b><span>Tracing</span></button>
        <button class="soon" disabled role="listitem"><b>▤</b><span>Book Builder</span><em>WKRÓTCE</em></button>
      </div>
    </div>`;
  body.appendChild(bar);

  const handle=bar.querySelector('.fpm-bar-handle'),content=bar.querySelector('.fpm-bar-content');let open=false,autoTimer=null;
  ...
  function go(target){
    ...
    if(target==='tracing'){if(!isTracing)setTimeout(()=>location.href='tracing.html',180);return}
    if(isStudioPage){setTimeout(()=>{location.href=target==='home'?'index.html':`index.html#${target}`},180);return}
    const legacy=document.querySelector(`[data-view="${target}"]`)||document.querySelector(`[data-go="${target}"]`);if(legacy)legacy.click();
    if(target!=='home')history.replaceState(null,'',`#${target}`);else history.replaceState(null,'',location.pathname);
  }
  handle.addEventListener('click',()=>setOpen(!open));bar.querySelectorAll('[data-shell-target]').forEach(btn=>btn.addEventListener('click',()=>go(btn.dataset.shellTarget)));
  if(!isStudioPage&&location.hash){const target=location.hash.slice(1);if(['home','assets','pages','maze'].includes(target))setTimeout(()=>go(target),0)}
})();