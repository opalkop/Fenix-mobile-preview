...
export function createMazeStudio({onCartChange=()=>{},onSaved=()=>{}}={}){
 ...
 function recipe(includePreview=true){const p=page(),meta={};if(includePreview){try{meta.previewImage=canvas.toDataURL("image/jpeg",.68)}catch(error){console.warn("Maze preview could not be saved",error)}}return createRecipe({...p,meta})}
 function saveToCart(){draw();let result=null;try{const r=recipe(true);result=editingPageId?CartManager.update(editingPageId,r):CartManager.add(r)}catch(error){console.warn("Saving maze with preview failed; retrying without preview",error);try{const r=recipe(false);result=editingPageId?CartManager.update(editingPageId,r):CartManager.add(r)}catch(secondError){console.error("Maze cart save failed",secondError);status.textContent="Nie udało się zapisać strony. Odśwież aplikację i spróbuj ponownie.";alert("Nie udało się dodać strony do Koszyka Feniksa.");return}}
 if(!result){status.textContent="Nie udało się zapisać strony w aktywnym projekcie.";return}
 if(editingPageId){status.textContent="Zmiany zapisane — Koszyk odświeżony";resetEditing();onCartChange();onSaved()}else{status.textContent="Dodano do Koszyka";onCartChange()}}
