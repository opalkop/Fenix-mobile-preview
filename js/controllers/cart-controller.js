...
export function createCartController({router,editors}){
  ...
  function openEditor(page){
    if(!page)return;
    const view=ModuleRegistry.editorView(page);
    const editor=editors[page.module];
    if(!view||!editor?.loadRecipe)return alert("Ten moduł nie ma jeszcze edytora mobilnego.");
    AppState.beginEdit(page.id);
    if(editor.loadRecipe(page))router.show(view);
  }
  ...
  function render(){
    ...
    box.innerHTML=pages.length?`<div class="cart-grid">${pages.map((page,index)=>`<article class="page-card"><button class="page-preview" data-edit="${page.id}"><span class="page-number">${index+1}</span><canvas data-thumb="${page.id}"></canvas></button><div class="page-info"><strong>${esc(page.title||"Strona")}</strong><small>${esc(ModuleRegistry.displayName(page.module))}</small></div><div class="page-actions"><button class="secondary" data-up="${page.id}">↑</button><button class="secondary" data-down="${page.id}">↓</button><button class="secondary" data-copy="${page.id}">Kopia</button><button class="danger" data-remove="${page.id}">Usuń</button></div></article>`).join("")}</div>`:"<p class='empty'>Koszyk jest pusty.</p>";
