...
export const TransferManager={
  build(project=ProjectManager.ensure()){
    return{type:"FENIX_MOBILE_PROJECT",formatVersion:FORMAT_VERSION,appVersion:APP_VERSION,createdAt:new Date().toISOString(),project:structuredClone(project)};
  },
  ...
  import(data,{asCopy=true}={}){
    this.validate(data);
    const source=structuredClone(data.project),project={...source,id:asCopy?uid("project"):source.id||uid("project"),name:asCopy?`${source.name||"Projekt"} — import`:(source.name||"Projekt"),createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),pages:(source.pages||[]).map(page=>({...page,id:uid("page")}))};
