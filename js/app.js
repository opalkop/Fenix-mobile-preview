...
import{createMazeStudio}from"./modules/maze.js?v=0.10.2-alpha11";
...
const editors={};editors["maze-studio"]=createMazeStudio({onCartChange:renderShell,onSaved:afterEditorSave});editors["coloring-studio"]=createColoringStudio({onCartChange:renderShell,onSaved:afterEditorSave});editors["tracing-studio"]=createTracingStudio({onCartChange:renderShell,onSaved:afterEditorSave});editors["logic-studio"]=createLogicStudio({onCartChange:renderShell,onSaved:afterEditorSave});projectsController=createProjectsController({router});cartController=createCartController({router,editors});initSettingsAndTransfer({router});[APP_EVENTS.PROJECT_CREATED,APP_EVENTS.PROJECT_UPDATED,APP_EVENTS.PROJECT_DELETED,APP_EVENTS.PROJECT_ACTIVATED,APP_EVENTS.CART_CHANGED,APP_EVENTS.IMPORT_COMPLETED,APP_EVENTS.SETTINGS_CHANGED].forEach(event=>EventBus.on(event,renderShell));ProjectManager.ensure();AppState.markReady();renderShell();router.show("home",{scroll:false});